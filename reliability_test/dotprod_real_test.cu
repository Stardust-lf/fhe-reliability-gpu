...
// ------------------------------------------------------------------
// Inject a random bit-flip anywhere in the ciphertext buffer
// ------------------------------------------------------------------
void inject_bitflip_ciphertext(PhantomCiphertext &ct) {
    ...
    size_t idx = rand() % total;    // random coefficient index
    size_t bit = rand() % 64;       // random bit position

    uint64_t *dptr = ct.data();     // raw device pointer to all coeffs

    // launch 1×1 kernel to flip that bit
...
void dot_product_test(PhantomContext &context) {
    ...
    // // 4) Inject a bit-flip fault into x_cipher
    // inject_bitflip_ciphertext(x_cipher);
    ...
    // Prepare CPU baseline for elementwise product

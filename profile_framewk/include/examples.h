...
inline void print_parameters(const seal::SEALContext &context)
{
    ...
    */
    ...
    std::cerr << "/" << std::endl;
    std::cerr << "| Encryption parameters :" << std::endl;
    std::cerr << "|   scheme: " << scheme_name << std::endl;
    std::cerr << "|   poly_modulus_degree: " << context_data.parms().poly_modulus_degree() << std::endl;

    /*
    Print the size of the true (product) coefficient modulus.
    */
    std::cerr << "|   coeff_modulus size: ";
    std::cerr << context_data.total_coeff_modulus_bit_count() << " (";
    auto coeff_modulus = context_data.parms().coeff_modulus();
    std::size_t coeff_modulus_size = coeff_modulus.size();
    for (std::size_t i = 0; i < coeff_modulus_size - 1; i++)
    {
        std::cerr << coeff_modulus[i].bit_count() << " + ";
    }
    std::cerr << coeff_modulus.back().bit_count();
    ...
    */
    ...
    {
        std::cerr << "|   plain_modulus: " << context_data.parms().plain_modulus().value() << std::endl;
    }

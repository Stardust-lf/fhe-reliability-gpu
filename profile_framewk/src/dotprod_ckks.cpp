    ...
    size_t            slot_count)
{
    ...
    cout << "frontend: MULTIPLY_CKKS[" << time_diff.count() << " microseconds]" << endl;
...
int main(int argc, char *argv[])
{
    ...
    std::vector<int> modulus_bits(24, 30);
    ...
    if (!context.parameters_set())
    {
        ...
        cerr << "Invalid EncryptionParameters (" 
             << error_name << "): " 
             << error_msg << endl;
        return 1;
    }
    ...
    // 离线计算点积（float）
    float expected_dot_f = inner_product(raw1.begin(), raw1.end(), raw2.begin(), 0.0f);
    ...
    Ciphertext enc_result = compute_dot_product(
        evaluator, c1, c2, relin_keys, gal_keys, slot_count);
    auto end = chrono::high_resolution_clock::now();
    auto duration = chrono::duration_cast<chrono::microseconds>(end - start);
    cout << "Encrypted dot product time: "
    ...
    cout << "Encrypted dot (slot 0) = " << encrypted_dot << endl;
    cout << "Expected dot           = " << expected_dot << endl;
    cout << "Difference (enc - exp) = " << diff << endl;
